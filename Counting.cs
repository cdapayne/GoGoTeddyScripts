...
public class Counting : MonoBehaviour
{
    ...
    public void PickRandomQuestion()
    {
        ...
        string varb = quest[QuestNum].pVari;
        ...
        //StartCoroutine(PlayAudio(Question,InitalNumber));
    ...
    IEnumerator PlayAudio(string Question, int Num)
    {
    ...
    public void ItemPress(GameObject NewItem)
    {
        StopAllCoroutines();
        if (!NewItem.GetComponent<Image>().color.Equals(Color.yellow))
        {
            StartCoroutine(SayNumber());
            NewItem.GetComponent<Image>().color = Color.yellow;
        }
    }
    IEnumerator SayNumber()
    {
        CurrentNumber = CurrentNumber + 1;
        BigNum.GetComponent<Text>().text = CurrentNumber.ToString();
        BigNum.SetActive(true);
        AudioClip add = Resources.Load(CurrentNumber.ToString()) as AudioClip;
        voice.clip = add;
        voice.Play();


        yield return new WaitForSeconds(1.0f);
        BigNum.SetActive(false);

        if (CurrentNumber == InitalNumber+1 && CurrentNumber<11)
        {
    ...
    public void ArrayLoad()
    {
        try
        {
            ...
            foreach (string Ques in questions)
            {
                ...
                {
                    pQuestion = "Touch to count the "+Ques+"s",
                    pVari = Ques,
                    pRightAnswer = "",
                    pWrongAnswer1 = "",
                    pWrongAnswer2 = ""
